...
public sealed class VvXqiuSpider : ISpider
{
    ...
    public ISpiderResult GetResult(string url)
    {
        ...
        string? json = SpiderRequest.Get($"https://h5p.vvxqiu.com/activity-center/fanclub/activity/captain/banner?roomId={Uri.EscapeDataString(roomId)}&product=vvstar", Headers(), PlatformCookieStore.GetCookie("VVXqiu", SecretProtector.GetChinaCookie()));
        ...
        string? response = SpiderRequest.Get(hlsUrl, Headers(), PlatformCookieStore.GetCookie("VVXqiu", SecretProtector.GetChinaCookie()));
